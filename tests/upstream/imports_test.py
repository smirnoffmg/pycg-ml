...
from pycg_ml.machinery.imports import (
    ImportManager,
    ImportManagerError,
    get_custom_loader,
)
...
class ImportsTest(TestBase):
    ...
    def test_hooks(self):
        ...
        with mock.patch(
            ...
            return_value=lambda path: custom_loader,
        ):
            im.install_hooks()

        installed = sys.path_hooks[0]
        pkg_root = os.path.abspath("somedir/somedir")
        self.assertEqual(installed(pkg_root), custom_loader)
        self.assertEqual(installed(os.path.join(pkg_root, "sub")), custom_loader)
        # The stub loader must stay out of everything outside the analysed package,
        # otherwise the analyser's own imports get served empty sources.
        with self.assertRaises(ImportError):
            installed(os.path.abspath("somedir"))
