...
def get_custom_loader(ig_obj):
    ...
    class CustomLoader(abc.SourceLoader):
        def __init__(self, fullname, path):
            ...
            # Since Python 3.12 importlib.invalidate_caches() itself imports
            # importlib.metadata, so the hook fires before any module of the
            # analysed program is current. Such imports belong to the analyser,
            # not to the import graph being built.
            if ig_obj.has_current_module():
...
class ImportManager:
    ...
    def has_current_module(self):
        return bool(self.get_node(self._get_module_path()))
    ...
    def _package_only(self, hook):
        """Confine the stub loader to the analysed package.

        The loader reports empty sources, which is harmless for the package under
        analysis but breaks any real import the analyser itself needs — and since
        Python 3.12 the standard library is imported lazily, well after the hook
        is in place.
        """
        pkg_root = os.path.abspath(self.mod_dir)

        def hook_within_package(path):
            resolved = os.path.abspath(path)
            if resolved != pkg_root and not resolved.startswith(pkg_root + os.sep):
                raise ImportError(path)
            return hook(path)

        return hook_within_package
